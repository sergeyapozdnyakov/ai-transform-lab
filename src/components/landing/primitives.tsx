...
export function StatusPill({ children, color = "indigo" }: { children: ReactNode; color?: "indigo" | "teal" }) {
  ...
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-[var(--color-border-emphasis)] bg-[var(--color-bg-card)] px-3 py-1.5">
      ...
      <span className="font-mono text-[10px] tracking-[0.12em] uppercase text-[var(--color-text-secondary)]">{children}</span>
    </span>
  );
}
...
export function CornerMeta({ children }: { children: ReactNode }) {
  return <div className="absolute bottom-3 right-4 font-mono text-[10px] tracking-[0.12em] uppercase text-[var(--color-text-mono)]">{children}</div>;
}
