...
const fmt = (n: number, dec = 0) => {
  const v = dec === 0 ? Math.round(n) : Math.round(n * 10 ** dec) / 10 ** dec;
...
export function Widget1BeforeAfter() {
  ...
  const flow = useMemo(() => {
    return t.hero.flowBefore.map((b, i) => ({
      ...
      // colour: end/start always neutral; middle (1-3) interpolate amber -> teal
  ...
  return (
    <div className="relative rounded-xl border border-[var(--color-border-emphasis)] bg-[var(--color-bg-elevated)]/80 p-5 backdrop-blur-sm">
        ...
        {t.hero.metrics.map((m, i) => {
          ...
          return (
            <div key={i} className="rounded-md border border-[var(--color-border-subtle)] bg-black/20 p-3">
              ...
              <div className="font-mono text-2xl text-[var(--color-text-primary)] tabular-nums">
      ...
      <div className="relative mt-4 h-12">
        <p
          className="absolute inset-0 text-[13px] leading-relaxed text-[var(--color-text-secondary)] transition-opacity duration-400"
          style={{ opacity: tt < 0.5 ? 1 : 0 }}
        >
          {t.hero.captionBefore}
        </p>
        <p
          className="absolute inset-0 text-[13px] leading-relaxed text-[var(--color-text-primary)] transition-opacity duration-400"
          style={{ opacity: tt >= 0.5 ? 1 : 0 }}
        >
          {t.hero.captionAfter}
        </p>
